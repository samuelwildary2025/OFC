    ...
    const loadInstance = async () => {
        try {
            ...
            if (response.data) {
                ...
                if (data.waPhoneNumberId) setPhoneNumberId(data.waPhoneNumberId);
                if (data.waBusinessAccountId) setBusinessAccountId(data.waBusinessAccountId);
            }
        } catch (error) {
    ...
    const handleConnect = async () => {
        ...
        try {
            if (!phoneNumberId || !accessToken) {
                toast.error('Preencha o ID do Telefone e o Token de Acesso');
                setConnecting(false);
                return;
            }

            await api.updateInstanceCredentials(id, {
                waPhoneNumberId: phoneNumberId,
                waAccessToken: accessToken,
                waBusinessAccountId: businessAccountId || undefined
            });

            toast.success('Credenciais salvas! Verificando conexão...');

            // Allow some time for backend to verify
            ...
            // Stop polling after 30 seconds (quick verify)
                            ...
                            {isConnected ? (
                                ...
                                <div className="space-y-4">
                                    <FacebookLogin
                                        ...
                                        className="w-full"
                                        onSuccess={() => {
                                            toast.success('Conexão iniciada. Verificando...');
                                            setTimeout(loadInstance, 2000);
                                        }}
                                        onError={(message) => {
                                            toast.error(message);
                                    ...
                                    <div className="text-xs text-[var(--muted)] text-center">
                                        ou conecte com credenciais abaixo
                                    </div>
                                    {/* Official API Credentials Form */}
                                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800 mb-4">
                                        ...
                                        <p className="mt-1">
                                            Para conectar, você precisa criar um App no painel de desenvolvedor da Meta e obter as credenciais.
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="text"
                                            placeholder="Ex: 104xxxxxxxxxxxxx"
                                            value={phoneNumberId}
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            Token de Acesso (Permanente ou Temporário)
                                        </label>
                                        <div className="relative">
                                            <input
                                                ...
                                                className="w-full bg-[var(--background)] border border-[var(--border)] rounded-lg px-3 py-2 pr-10"
                                            />
                                        </div>
                                        <p className="text-xs text-[var(--muted)] mt-1">
                                            Recomendado usar um token de Sistema (Permanente).
                                        </p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            ID da Conta do WhatsApp Business (Opcional)
                                        </label>
                                        <input
                                            type="text"
                                            placeholder="Ex: 102xxxxxxxxxxxxx"
